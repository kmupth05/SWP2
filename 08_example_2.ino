...
void loop() {
  ...
  if ((distance > 300) || (distance < 100)) {
      ...
      bright = distance - 200.0;    // Set Lower Value
      bright=int(bright)*int(255/100);
      analogWrite(PIN_LED, bright);       // LED OFF
  } else if (distance < 200) {
      bright = distance - 100.0;    // Set Lower Value
      bright=200-(int(bright)*int(255/100));
      analogWrite(PIN_LED, bright);       // LED OFF
  } else {
